...
class RecordingControllerTest {
    ...
    @Test
    fun `resumeIfPausedFor ignores an unrelated reason and only resumes on a match`() = runTest {
        Mockito.mockStatic(SystemClock::class.java).use { mockedClock ->
            mockedClock.`when`<Long> { SystemClock.elapsedRealtime() }.thenReturn(1_000L)
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val controller = RecordingController(
            ...
            whenever(mockRepository.createNewSession()).thenReturn(1L)
            whenever(mockRepository.getSessionWithChunks(1L)).thenReturn(null)

            controller.start()
            runCurrent()
            controller.pause(PauseReason.USER)
            runCurrent()
            assertEquals(RecordingStatus.Paused(PauseReason.USER), controller.recordingStatus.value)

            controller.resumeIfPausedFor(PauseReason.AUDIO_FOCUS)
            runCurrent()
            assertEquals(RecordingStatus.Paused(PauseReason.USER), controller.recordingStatus.value)

            controller.resumeIfPausedFor(PauseReason.USER)
            runCurrent()
            assertEquals(RecordingStatus.Recording, controller.recordingStatus.value)
