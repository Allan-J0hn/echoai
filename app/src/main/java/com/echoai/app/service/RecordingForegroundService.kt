package com.echoai.app.service
...
import com.echoai.app.MainActivity
import com.echoai.app.R
import com.echoai.app.domain.RecordingController
import com.echoai.app.domain.RecordingStatus
import com.echoai.app.workers.FinalizeSessionWorker
...
class RecordingForegroundService : Service(), AudioManager.OnAudioFocusChangeListener {
    ...
    private fun observeAudioDevices() {
        audioDeviceCallbackHandler.register { hasInputDevice ->
            ...
            if (!hasInputDevice && (status is RecordingStatus.Recording || status is RecordingStatus.Warning)) {
                ...
                serviceScope.launch { recordingController.resumeIfPausedFor(PauseReason.AUDIO_DEVICE) }
            }
        }
    }
    ...
    override fun onAudioFocusChange(focusChange: Int) {
        when (focusChange) {
            ...
            AudioManager.AUDIOFOCUS_GAIN -> serviceScope.launch { recordingController.resumeIfPausedFor(PauseReason.AUDIO_FOCUS) }
        }
    }
    ...
    private fun onCallStateChanged(state: Int) {
        when (state) {
            ...
            TelephonyManager.CALL_STATE_IDLE -> serviceScope.launch { recordingController.resumeIfPausedFor(PauseReason.PHONE_CALL) }
        }
    }
    ...
    companion object {
        const val ACTION_START = "com.echoai.app.service.START"
        const val ACTION_PAUSE = "com.echoai.app.service.PAUSE"
        const val ACTION_RESUME = "com.echoai.app.service.RESUME"
        const val ACTION_STOP = "com.echoai.app.service.STOP"
